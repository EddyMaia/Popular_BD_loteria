...
#percorre um arquivo txt populando a tabela de resultados conforme o tipo de jogo
    ...
    #lê o txt obtendo o total de linhas e quantas vezes deve percorrer para apurar o arquivo inteiro
        self.caminho_arquivo=input('digite aqui o caminho do arquivo: ').strip()
        lista=[]
        vezes_percorrer=0
        try:
            ...
            print("ocorreu um erro ao localizar o caminho digitado")
    ...
    #percorre o arquivo organizando os resultados e inserindo no banco
        ...
        with open(self.caminho_arquivo,'r') as arquivo:
            ...
            while vezes < total_vezes :
                ...
                linha_concurso=linha_concurso.replace('Resultado Concurso', '')
                linha_concurso=linha_concurso.strip()
                linha_concurso=linha_concurso.split()
                
                concurso=linha_concurso[0]
                #conpara se ouve repetição
                if concurso != concurso_anterior:
                
                    str_resultado=""
                    inicio_leitura=0
                    
                    #percorre a linha que contém o resultado
                    for x in range(self.total_num_sorteado + 2):
                    ...
                    if self.ultimo_concurso == concurso:
                    #insert no banco
                        cur.execute(sql+"resultados(id_concurso,resultado) values(%s,%s);",(self.ultimo_concurso,str_resultado))
                        conectar.commit()
                    else:
                        arquivo_correcao=open('correcao_'+ self.prefixo_tabela,'a')
                        arquivo_correcao.write(concurso +'é diferente do esperado: '+self.ultimo_concurso)
                else:
                    #escreve no arquivo caso concurso esteja repetido
                    arquivo_correcao=open('correcao_'+ self.prefixo_tabela,'a')
                    arquivo_correcao.write(concurso +'repetiu')
